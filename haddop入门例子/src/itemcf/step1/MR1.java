 ...
 *         输出：用户ID（行）-物品ID（列）-分值
 * 
 *         4.物品与物品相似度矩阵*评分矩阵（经过步骤3转置） 
 *         输入：步骤2的输出 
 *         缓存：步骤3的输出 
 *         输出：物品ID（行）-用户ID（列）-分值
 * 
 *         5.根据评分矩阵，将步骤4的输出中，用户已经有过行为的商品评分为置为0 
 *         输入：步骤4的输出 
 *         缓存：步骤1的输出
...
public class MR1 {
	// 输入相对路径
	private static String inPath = "/user/hyc/itemcf/itemcf_input/itemcf.txt";
	// 输出相对路径
	private static String outPath = "/user/hyc/itemcf/itemcf_output1";
